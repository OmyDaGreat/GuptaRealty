...
import xyz.malefic.guptarealty.client.api.getWebinarRegistrations
import xyz.malefic.guptarealty.client.api.getWebinarReviews
import xyz.malefic.guptarealty.client.api.getWebinarTips
import xyz.malefic.guptarealty.client.api.postWebinar
import xyz.malefic.guptarealty.client.api.postWebinarReviews
import xyz.malefic.guptarealty.client.api.postWebinarTips
...
import xyz.malefic.guptarealty.client.components.DateTimeSelector
...
import xyz.malefic.guptarealty.client.styles.ContentCardStyle
...
import xyz.malefic.guptarealty.client.styles.TertiaryButtonStyle
    ...
    Column(Modifier.fillMaxSize().overflow(Overflow.Auto).padding(AppSpacing.S4)) {
        ...
        Column(ContentCardStyle.toModifier().fillMaxWidth().margin(bottom = AppSpacing.S4)) {
            Loading(webinar) {
                H2(HeadlineSmStyle.toModifier().margin(bottom = AppSpacing.S3).toAttrs()) {
                    Text("Main Webinar Info")
                }
                AdminField("Header", header) {
                    webinar = webinar?.copy(header = it)
                }
                AdminField("Title", title) {
                    webinar = webinar?.copy(title = it)
                }
                                ...
                                scope.launch {
                                    message =
                                        try {
                                            ...
                                            "Webinar updated successfully" to true
                                        } catch (e: Exception) {
                                            Logger.e(e, "Webinar") { "Failed to update webinar" }
                                            "Failed to update webinar" to false
                                        }
                                }
                            }
                        },
        ...
        Column(ContentCardStyle.toModifier().fillMaxWidth().margin(bottom = AppSpacing.S4)) {
            Loading(registrations) {
                ...
                if (this.isEmpty()) {
                    P(LabelMdStyle.toModifier().toAttrs()) {
                        Text("No registrations accessible.")
                    }
                } else {
                    this.forEach { registration ->
                        Row(
                            ...
                            Column(Modifier.weight(1f)) {
                                H3(LabelMdStyle.toModifier().toAttrs()) {
                            ...
                            P(LabelSmStyle.toModifier().toAttrs()) {
                                Text(
                                    registration.phone,
                                )
                            }
                ...
                Row(
                    ...
                    (0..2).forEach { index ->
                        ...
                        Column(
                            ...
                            H2(LabelMdStyle.toModifier().margin(bottom = AppSpacing.S2).toAttrs()) {
                                Text("Tip #${index + 1}")
                            }
                                ...
                                scope.launch {
                                    message =
                                        try {
                                            ...
                                            while (finalTips.size < 3) {
                                                finalTips.add(WebinarTip("help", "New Tip", "Description..."))
                                            }
                    ...
                    this.forEachIndexed { index, review ->
                        Column(
                            ...
                            Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                                H2(LabelMdStyle.toModifier().toAttrs()) {
                                    Text("Review #${index + 1}")
                                }
                                    ...
                                    scope.launch {
                                        message =
                                            try {
                                                ...
                                                "Reviews updated successfully" to true
                                            } catch (e: Exception) {
                                                Logger.e(e, "Webinar") { "Failed to update reviews" }
                                                "Failed to update reviews" to false
                                            }
                                    }
                                }
                            },
