    ...
    Column(Modifier.fillMaxSize().overflow(Overflow.Auto).padding(AppSpacing.S4)) {
        ...
        Column(
            ...
            Loading(settings) {
                ...
                AdminField("Email", contactEmail) {
                    settings = settings?.copy(contactEmail = it)
                }
                AdminField("Phone", contactPhone) {
                    settings = settings?.copy(contactPhone = it)
                }
                AdminField("Address", contactAddress) {
                    settings = settings?.copy(contactAddress = it)
                }
                AdminField("DRE#", dreNumber) {
                    settings = settings?.copy(dreNumber = it)
                }
